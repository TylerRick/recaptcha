...
#    end
  ...
  describe 'v3' do
    ...
    describe "#verify_recaptcha_v3" do
      ...
      describe 'VerifyResult#score_above_threshold?' do
        before do
          expect_http_post.to_return(body: success_body(score: 0.4))
        end

        it "fails when score is below score_threshold" do
          assert_result_error verify_recaptcha(score_threshold: 0.5), "Score 0.4 was below score threshold 0.5"
          assert_equal "reCAPTCHA verification failed, please try again.", @controller.flash[:recaptcha_error]
        end

        it "passes with score exactly at score_threshold" do
          assert verify_recaptcha(score_threshold: 0.4)
        ...
        it "passes with nil" do
          assert verify_recaptcha(score_threshold: nil)
          assert_equal true, @controller.recaptcha_verify_result.score_above_threshold?
          assert_nil @controller.flash[:recaptcha_error]
        end

        it "passes with false" do
          assert verify_recaptcha(score_threshold: false)
          assert_equal true, @controller.recaptcha_verify_result.score_above_threshold?
          assert_nil @controller.flash[:recaptcha_error]
        end
      end
